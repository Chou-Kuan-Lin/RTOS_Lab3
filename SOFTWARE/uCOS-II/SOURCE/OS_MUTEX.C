...
void  OSMutexPend (OS_EVENT *pevent, INT16U timeout, INT8U *err)
{
    ...
    if ((INT8U)(pevent->OSEventCnt & OS_MUTEX_KEEP_LOWER_8) == OS_MUTEX_AVAILABLE) {
        ...
        *err  = OS_NO_ERR;
        ...
        if((pip==1&&mprio==2))
            sprintf(msg, "%lu \t (1)LOCK \t R%u (Prio=%u changes to=%u)\n", OSTime, pip, pip, pip);
        else if((pip==2&&mprio==1))
            sprintf(msg, "%lu \t (2)LOCK \t R%u (Prio=%u changes to=%u)\n", OSTime, pip, mprio, mprio);
        else
            sprintf(msg, "%lu \t (3)lock \t R%u (Prio=%u changes to=%u)\n", OSTime, pip, mprio, pip);
        puts(msg);
...
INT8U  OSMutexPost (OS_EVENT *pevent)
{
    ...
    if((pip==1&&OSTCBCur->OSTCBPrio==2))
        sprintf(msg, "%lu \t (1)unlock \t R%u (Prio=%u changes to=%u)\n", OSTime, pip, pip, pip);
    else if((pip==2&&OSTCBCur->OSTCBPrio==1))
        sprintf(msg, "%lu \t (2)unlock \t R%u (Prio=%u changes to=%u)\n", OSTime, pip, OSTCBCur->OSTCBPrio, OSTCBCur->OSTCBPrio);
    else
        sprintf(msg, "%lu \t (3)unlock \t R%u (Prio=%u changes to=%u)\n", OSTime, pip, pip, OSTCBCur->OSTCBPrio);
    puts(msg);
